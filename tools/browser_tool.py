...
_BROWSER_PASSTHROUGH_KEYS: tuple[str, ...] = (
    ...
    # Local addition: proxy configuration for agent-browser. ``_scrub_credentials``
    # drops these along with the provider keys, so a proxied environment would
    # otherwise lose its egress route in the browser subprocess. Re-added here
    # through the same scope-aware path as the backend keys (never raw
    # ``os.environ``), so a routed profile still gets its own proxy config.
