...
def _windows_bash_candidates(custom: "str | None") -> list[str]:
    ...
    known Git-for-Windows dirs (including secondary drives D:/E:/F:, where Git for
    Windows is often installed), then PATH last — ``shutil.which`` may return WSL's
    ...
    roots = [
        ...
        # Secondary drives: on machines with both WSL and Git for Windows the
        # PATH lookup can return WSL's bash (a stub launcher that fails
        # silently on Windows paths). Git for Windows is frequently installed
        # on D:/E:/F: rather than C:, so probe those explicitly before PATH.
        r"D:\Git\bin",
        r"E:\Git\bin",
        r"F:\Git\bin",
