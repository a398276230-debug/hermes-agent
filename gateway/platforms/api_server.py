...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Local addition: live agents for /v1/chat/completions (and session-chat streaming)
        # keyed by X-Hermes-Session-Id, so chat clients can redirect a follow-up into the
        # running turn instead of dropping the connection (which hard-interrupts the agent).
        # Same lifecycle as ``_active_run_agents``: registered when the agent is created in
        # ``_run_agent``, popped when the turn returns. Served by
        # POST /v1/chat/redirect and POST /v1/chat/stop.
    ...
    def _http_route_table(self) -> List[tuple]:
        ...
        routes: List[tuple] = [
            ...
            # Local additions: mid-turn control for OpenAI-compatible chat clients, keyed by
            # X-Hermes-Session-Id (see ``_chat_session_agents``).
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    # Streaming chat surfaces (agent_ref callers) also expose the live agent
                    # by session_id so POST /v1/chat/redirect can fold a follow-up into this
                    # turn without the client dropping the SSE connection. Identity-guarded
                    # removal in the finally below keeps a same-session successor from being
                    # deregistered by its predecessor.
