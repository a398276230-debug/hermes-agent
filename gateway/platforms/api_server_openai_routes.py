...
class OpenAICompatRoutesMixin:
    ...
    async def _handle_chat_redirect(self, request: "web.Request") -> "web.Response":
        ...
        Local addition. OpenAI-compatible clients (e.g. chat bridges) drive Hermes through
        /v1/chat/completions with X-Hermes-Session-Id; their only mid-run lever until now was
        dropping the SSE connection, which the server treats as a hard interrupt
        ("Operation interrupted..."). This endpoint exposes the same active-turn redirect the
        native gateway surfaces use (gateway/run.py busy-input handling): the in-flight provider
        request is cancelled, the completed turn prefix is kept, the correction lands as a user
        message, and the run continues. During tool execution agent.redirect() degrades to
        steer() so no tool is killed just to deliver conversational guidance.
        """
        from gateway.platforms.api_server import (
            _normalize_chat_content, _openai_error, _redact_api_error_text)
        ...
        if agent is None or not hasattr(agent, "redirect"):
            return web.json_response(
                _openai_error(f"No live run for session: {session_id}", code="no_active_run"),
                status=409)
        ...
        try:
            ...
            return web.json_response(
                _openai_error(_redact_api_error_text(exc), code="redirect_failed"), status=500)
        if not accepted:
            # agent.redirect() returns False when there is no live model request and no executing
            # tool batch (turn finished, or a prior interrupt is already in flight). Tell the caller
            # so it can fall back to its next-turn queue instead of assuming the correction landed.
            return web.json_response(
                _openai_error(f"Run did not accept redirect text: {session_id}",
                              code="redirect_not_accepted"),
                status=409)
        return web.json_response({
            "object": "hermes.chat.redirect", "session_id": session_id, "accepted": True})

    async def _handle_chat_stop(self, request: "web.Request") -> "web.Response":
        """POST /v1/chat/stop — hard-stop a session's live run.

        Local addition, companion to /v1/chat/redirect: once a client folds follow-ups into the
        running turn it loses the old "drop the connection to interrupt" lever. This endpoint
        restores an explicit escape hatch — the live agent gets request_hard_interrupt (cascades
        to tools and subagents), mirroring /v1/runs/{run_id}/stop semantics for chat-completions
        sessions. Idempotent: stopping a session with no live run reports nothing to stop.
        """
        from gateway.platforms.api_server import _openai_error, request_hard_interrupt
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        if agent is None:
            return web.json_response({
                "object": "hermes.chat.stop", "session_id": session_id, "stopped": False,
                "detail": "No live run for this session."})

        try:
            request_hard_interrupt(agent, "Stop requested via chat API")
        except Exception:
            logger.exception("[api_server] stop failed for session %s", session_id)
            return web.json_response(
                _openai_error("Stop request failed.", code="stop_failed"), status=500)
        return web.json_response({
            "object": "hermes.chat.stop", "session_id": session_id, "stopped": True})
