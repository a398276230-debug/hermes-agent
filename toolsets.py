...
TOOLSETS = {
    ...
    # Local: Sideria Bridge meme library. The bridge owns storage/delivery; the
    # tool only queries its loopback HTTP API and returns stable meme ids. Gated
    # by ``check_meme_search_requirements``, so it stays out of the schema when
    # the bridge is not running.
    "meme_search": _ts("Search the local Sideria Bridge meme library", ["search_memes"]),
